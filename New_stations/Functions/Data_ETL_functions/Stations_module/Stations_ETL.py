...
def load_and_filter_data(path_to_file):
    ...
    :param path_to_file: path to csv file with columns: departure_id, return_id, distance (m), duration (sec.),
...
def prepare_time_series_data(input_dt, time_granularity_in_hours=2):
    ...
    :param input_dt: dataframe with raw bikes data
    :param time_granularity_in_hours: time between two timestamps
    :return: prepared time series dataframe
    """
...
def get_graph_metrics_for_network(df, threshold=10):
    """

    :param threshold:
    :param df: pandas dataframe with columns: departure_name, return_name, departure_latitude, departure_longitude
    ...
    df_grouped = df_grouped.loc[df_grouped.departure >= threshold, :]
