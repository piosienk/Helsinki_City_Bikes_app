...
def create_coordinates_grid(path="./Data/Bikes/", city_center_bound=None):
    ...
    :param path: Path to the main bike-sharing data
    :param city_center_bound: borders of area considered as city center

    Function creates new coordinates grid based on the existing stations data and city center boundaries.
    Points are built at 150m intervals. Grid area is extended from the existing network by 1.5km South, 2km North, 2km West
    and 2km East. Additional 1000 coordinates are randomly selected from the city center area.

    :return: dataframe with new coordinates from grid
    """
...
def check_if_water(df_coord, path="./Data/Locations/"):
    ...
    :param df_coord: dataframe with coordinates grid
    :param path: path to the directory with locations folder

    Using REST API, function flags points that refer to water location. The whole process from request creation to
    saving response in the dataframe is executed in this functions.

    :return: dataframe that consists of coordinates and flag whether particular coordinate is a point on water or land
    """
    ...
    while batch_n <= batches_total:
        ...
        if batch_n % 1000 == 0:
            path_temp_file = "./Data/Temp/" + str(batch_n) + "_batch_is_water.csv"
...
def get_distance_to_other(df_all_new):
    ...
    :param df_all_new: dataframe with points from search grid

    For each point in grid, function calculates distance (absolute) to the nearest point in grid (in km)

    :return: dataframe with added distance to the nearest point in grid
    """

    for i in range(len(df_all_new)):
        print(i)
        for j in range(max(0, i - 1000), min(len(df_all_new), i + 1000)):
            if j != i:
